...
// error instead of returning it from the daemon's service group. The docker
// build intentionally omits fuseftp, but the same binary can also be installed
// as an ordinary client where intercept.useFtp is still configured.
