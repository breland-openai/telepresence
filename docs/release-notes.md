...
## <div style="display:flex;"><img src="images/bugfix.png" alt="bugfix" style="width:30px;height:fit-content;"/><div style="display:flex;margin-left:7px;">Give cold named proxy agents time to start</div></div>
<div style="margin-left: 15px">

Initial connections and reconnections using <code>--proxy-via</code> now allow each distinct named workload the configured <code>timeouts.intercept</code> for its traffic-agent to start. Named proxy connections also fail early with reconnect guidance if agents in the connected namespace cannot be watched.
</div>
